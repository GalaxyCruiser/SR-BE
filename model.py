...
class GNN(Module):
    def __init__(self, hidden_size, step=1):
        ...
        self.w_t = Parameter(torch.Tensor(self.hidden_size, self.input_size))
        self.b_t = Parameter(torch.Tensor(self.hidden_size))
...
class SR_BE(Module):
    ...
    def forward(self, inputs, A):
        ...
        hidden2 = self.gnn(A, hidden)
        hidden1 = hidden.transpose(0,1).contiguous()
        hidden1 = self.transformerEncoder(hidden1)
        hidden1 = hidden1.transpose(0,1).contiguous()
        hidden = hidden1*self.weight + (1-self.weight)*hidden2
